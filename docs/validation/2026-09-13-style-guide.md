...
P1 spacing, P2 shell geometry/elevation, P3 selected/focus distinction and P4 focus restoration are review proposals. The guide's target specimen is separate from the production examples.
...
- Palette selection updates light/dark previews. Numeric End reaches 20 units and disables increment. Changing swatch selection retains the selected mark when keyboard focus moves to the proposed specimen.
...
Review from 01 Foundations upward. Agree on the four target proposals before tracking and implementing corresponding production migrations. FER-77 remains In Review through user review; this local guide does not imply that the source colors PR has been merged.
