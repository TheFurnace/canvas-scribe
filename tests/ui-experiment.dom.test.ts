...
it("keeps all radial colors and More colors reachable, cancel transactional, and page focus stable", () => {
  ...
  const colorIds = () => Array.from(document.querySelectorAll<HTMLElement>('[data-color]')).map((node) => node.dataset.color);
  ...
  expect(initialRecent).toHaveLength(3);
  expect(document.querySelectorAll('.canvas-scribe-arc-color:not([hidden])')).toHaveLength(10);
  expect(document.querySelector('.canvas-scribe-radial-title')).toBeNull();
  document.querySelector<HTMLButtonElement>('[data-color="#dc2626"]')!.click();
  ...
  expect(document.querySelector('[data-color="#dc2626"]')?.getAttribute("aria-pressed")).toBe("true");
  expect(document.querySelector('[data-action="default-color"]')).toBeNull();
  document.querySelector<HTMLButtonElement>('[data-color="#2563eb"]')!.click();
