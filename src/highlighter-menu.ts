...
export function createHighlighterMenu(document: Document, options: {
  ...
  const choices = HIGHLIGHTER_TYPES.map((value) => {
    ...
    options.renderIcon(icon, toolIconId(`highlighter-${value}`, "tip"));
  ...
  root.append(preview, createAction(document, "Colors…", options.onColors));
  function sync() {
    ...
    for (const [x, y] of [[12, 42], [120, 55]]) {
      ...
      path.setAttribute("d", strokeToSvgPath({
        id: "preview", tool: "highlighter", highlighterType: type, color: options.color,
        size, opacity, hasPressure: false, createdAt: 0,
        points: [{ x: x!, y: y!, pressure: 0.5, time: 0 }, { x: 258, y: y!, pressure: 0.5, time: 1 }],
      }));
      path.setAttribute("fill", options.color); path.setAttribute("opacity", String(opacity));
      preview.append(path);
    }
  }
  sync(); return root;
