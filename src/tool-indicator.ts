...
export function toolDescription(tool: InkTool, type: PenType, color: string, size?: number, opacity?: number, isDefault = color.startsWith("var(")): string {
  return [tool === "pen" ? `${PEN_PROFILES[type].label} pen` : "Highlighter",
    isDefault ? `${defaultColorLabel(tool)} color` : color,
