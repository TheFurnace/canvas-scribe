...
// Review proposals only. Production collections await the user's curation.
const pen = [
...
function preview() {
  ...
  const heading = document.createElement("h2"); heading.textContent = "Color sets — curation review"; root.append(heading);
  const intro = document.createElement("p"); intro.textContent = "Proposed sets, not yet applied. Theme and Default remain separate choices. Select a swatch to compare ink on light and dark backgrounds."; root.append(intro);
