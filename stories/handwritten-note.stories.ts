...
function fixture(long = false): HandwrittenNoteDocument {
  ...
  note.objects = [
    ...
    { kind: "text", id: "text-one", x: 120, y: 300, width: 500, text: "Ordinary text stays editable. [[Link-like text]] remains plain content.", fontSize: 22, color: "#1f2937", align: "left" },
  ];
  if (long) for (let i = 1; i <= 10; i += 1) note.objects.push({ kind: "text", id: `section-${i}`, x: 120, y: 400 + i * 430, width: 660, text: `Long-note section ${i}\nInk and text retain stable document coordinates while the viewport scrolls.`, fontSize: 20, color: "#1f2937", align: "left" });
  return note;
}

function editorStory(long = false): HTMLElement {
  const host = document.createElement("div"); host.style.cssText = "height:720px;min-height:520px";
...
const meta = { title: "Canvas Scribe/Handwritten Note", render: () => editorStory() } satisfies Meta;
export default meta;
